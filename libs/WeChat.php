...
class WeChat {
    ...
    public static function inst() {
        if (self::$instance == null) {
            $appid = setting::instance()->load("KEY_WECHAT_APPID");
            $secret = setting::instance()->load("KEY_WECHAT_APPSECRET");
            self::$instance = new WeChat($appid, $secret);
        }
    ...
    private function __construct($appid, $secret) {
        ...
        $this->tokenfile = "/tmp/wechat_token." . $this->appid . ".txt";
        $this->ticketfile = "/tmp/wechat_ticket." . $this->appid . ".txt";
        $this->apilock = new Lock("wechat_api_lock_" . $this->appid);
    }
...
//            file_put_contents("/tmp/error/errorLog" . date("Y-d-m") . ".txt", date("Y-m-d H:i:s") . "   {$function}   {$out}\n", FILE_APPEND);
    ...
    public function doOAuth($userinfo = false) {
        ...
        if ($code == null) {
            // $currentUri = ROOT_URL . $_SERVER["REQUEST_URI"]; // substr($_SERVER['SCRIPT_NAME'], 1); //  . '?' . $_SERVER['QUERY_STRING'];
            $currentUri = get_current_url();

            // $currentUri = "http://sg.wuziyi.cc/test/t.html";
    ...
    private function get_all_users_interlocked($access_token, $arg) {
        $next = "";
        $users = array();
        do {
            $extra = "";
            if (!empty($next)) {
                $extra = "&next_openid=$next";
            }
            $url = "https://api.weixin.qq.com/cgi-bin/user/get?access_token=$access_token$extra";
            $plain = $this->read($url);
            $ret = json_decode($plain, true);
            if (isset($ret["errcode"])) {
                return $plain;
            }
            if (isset($ret["data"]["openid"])) {
                foreach ($ret["data"]["openid"] as $user) {
                    $users[] = $user;
                }
            }
            if (isset($ret["next_openid"])) {
                $next = $ret["next_openid"];
            }
        } while (!empty($next));
    ...
    public function get_all_users() {
        return $this->run_and_check_api('get_all_users_interlocked', array());
    }


    private function get_user_info_interlocked($access_token, $openid) {
        if (is_array($openid)) {
            $result = array();

            $url = "https://api.weixin.qq.com/cgi-bin/user/info/batchget?access_token=$access_token";
            $chunks = array_chunk($openid, 100);
            foreach ($chunks as $openids) {
                $data = array("user_list" => array());
                foreach ($openids as $oid) {
                    $data["user_list"][] = array("openid" => $oid, "lang" => "zh_CN");
                }
                $plain = $this->read($url, json_encode($data));
                $ret = json_decode($plain, true);
                if (isset($ret["errcode"])) {
                    return $plain;
                }
                if (isset($ret["user_info_list"])) {
                    foreach ($ret["user_info_list"] as $userinfo) {
                        $result []= $userinfo;
                    }
                }
            }
    ...
    public function get_user_info($openid) {
        return $this->run_and_check_api('get_user_info_interlocked', $openid);
    }
}
