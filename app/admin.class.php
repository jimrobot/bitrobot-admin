...
class Admin {
    ...
    public function save() {
        $id = $this->id();
        if ($id == 0) {
            $id = db_admin::inst()->add($this->username(), $this->password(), $this->nickname(), $this->telephone(), $this->email(), $this->mSummary["groups"], $this->comments());
            if ($id !== false) {
                $this->mSummary["id"] = $id;
            }
        } else {
            $id = db_admin::inst()->modify($id, $this->username(), $this->password(), $this->nickname(), $this->telephone(), $this->email(), $this->mSummary["groups"], $this->comments());
        }
        return $id;
    }
    ...
    public static function create($uid) {
        $user = db_admin::inst()->get($uid);
        return new Admin($user);
    }

    public static function all($include_deleted = false) {
        $users = db_admin::inst()->all();
        $arr = array();
        foreach ($users as $uid => $user) {
            if (!$include_deleted) {
                if ($user["status"] == db_admin::STATUS_DELETED) {
                    continue;
                }
            }
            $arr[$uid] = new Admin($user);
        }
        return $arr;
    }
    ...
    public static function remove($uid) {
        return db_admin::inst()->remove($uid);
    }
};
