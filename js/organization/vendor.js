$(document).ready(function() {
    ...
    var vendor = new Vue({
        ...
        data: {
            ...
            modal: {
                ...
                name: '',
                showname: '',
                category: VENDOR_CATEGORY_INDEX,
                telephone: '',
                email: '',
                address: '',
                comments: '',
        ...
        methods: {
            selectAll: function() {
                select_node(VENDOR_CATEGORY_INDEX);
            },
                ...
                if (this.modifyid == 0) {
                    ...
                    __request("api.v1.vendor.modify", {
                        id: this.modifyid,
                        name: this.modal.name,
                        title: this.modal.showname,
                        category: this.modal.category,
            ...
            edit: function(event) {
                ...
                var vid = this.vendors[vk].id;
                this.modifyid = vid;
                this.modal.title= "修改供应商";
                this.modal.name = this.vendors[vk].name;
                this.modal.showname = this.vendors[vk].title;
                this.modal.category = this.vendors[vk].category.id;
                this.modal.telephone = this.vendors[vk].telephone;
                this.modal.email = this.vendors[vk].email;
                this.modal.address = this.vendors[vk].address;
                this.modal.comments = this.vendors[vk].comments;
                this.modal.vprovince = this.vendors[vk].province;
                this.modal.vcity = this.vendors[vk].city;
                this.modal.vdistrict = this.vendors[vk].district;
                // console.debug(vid);
                $("#theModal").modal();
            },
            ...
            doRemove: function(event) {
                __request("api.v1.vendor.remove", {id: this.modifyid}, reload_data);
                $("#theModal").modal("hide");
            ...
            addNew: function(event) {
                ...
                this.modal.name = '';
                this.modal.showname = '';
                this.modal.category = VENDOR_CATEGORY_INDEX;
    ...
    var select_node = function(cid) {
        ...
        var currentNode = find_node(vendor.category, cid);
        enum_node(vendor.category, function(node) {
            node.selected = false;
            if (currentNode != null) {
        ...
        if (currentNode != null) {
            ...
            for (var k in vendor.vendors) {
                if (currentNode.id == vendor.vendors[k].category.id || (cid == VENDOR_CATEGORY_INDEX && vendor.vendors[k].category.id == 0)) {
                    vendor.vendors[k].show = true;
                    continue;
                }
                var flag = false;
                for (var k1 in currentNode.descendants) {
                    if (currentNode.descendants[k1] == vendor.vendors[k].category.id) {
                        vendor.vendors[k].show = true;
                        flag = true;
                        break;
                    }
                ...
                if (!flag) {
                    vendor.vendors[k].show = false;
                }
            }
        }
        vendor.currentNode = currentNode;
    }
    ...
    var reload_data = function() {
        __request("api.v1.vendor.list", {}, function(res) {
            ...
            vendor.vendors = res.data;
            if (vendor.currentNode != null) {
                select_node(vendor.currentNode.id);
            }
        });
    }
    ...
    __request("api.v1.category.list", {}, function(res) {
        ...
        // res.data.children[VENDOR_CATEGORY_INDEX - 1].name = "供应商";
        vendor.category = res.data.children[VENDOR_CATEGORY_INDEX - 1];

        categories = []; // [{name: '所有分类', id: 3}];
        enum_node(res.data.children[3], function(node) {
